...
def inference(images, inference_handle, nlabels, training=True):
    ...
    s = inference_handle(images, training, nlabels)
    tf.logging.set_verbosity(tf.logging.WARN)
...
def loss(logits, labels, nlabels, loss_type, weight_decay=0.0):
    ...
    elif loss_type == 'crossentropy_incomplete':
        class_weights = np.ones(nlabels)
        class_weights[0] = 0
        class_weights/=(len(class_weights) - 1)
        segmentation_loss = losses.pixel_wise_cross_entropy_loss_weighted(logits, labels, class_weights=class_weights)
...
def predict(images, inference_handle, nlabels):
    '''
    ...
    logits = inference_handle(images, training=tf.constant(False, dtype=tf.bool), nlabels=nlabels)
...
def evaluation(logits, labels, images, nlabels, loss_type, weak_supervision=False, cnn_threshold=None, include_bg=True):
    ...
    if weak_supervision:
        if not cnn_threshold is None:
            ...
            ####
            s = tf.multiply(tf.ones(shape=[10, 212, 212, 1]), threshold)
